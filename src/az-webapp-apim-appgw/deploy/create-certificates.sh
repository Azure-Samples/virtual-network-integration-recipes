...
create_server_cert () {
    ...
    openssl req -new -sha256 -key domain.key -out domain.csr -subj "/CN=$CUSTOM_DOMAIN" -addext "subjectAltName=DNS:$CUSTOM_DOMAIN" 
...
create_root_CA_cert () {
    ...
    openssl req -new -sha256 -key rootCA.key -out rootCA.csr -subj "/CN=Sample CA"
