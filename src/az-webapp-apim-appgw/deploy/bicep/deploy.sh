...
az deployment group create \
    ...
    apiManagementManagementCustomHostnameBase64EncodedPfxCertificate="$(cat ../.certs/domain.pfx.txt)" > output.json

if [[ $PEER_VNET == "true" ]]; then
    echo "Retrieving virtual network name created in Application Gateway with Private API Management and Private Web App recipe deployment . . ."
